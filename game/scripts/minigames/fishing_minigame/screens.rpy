init python:
    ...
    class Fishing(renpy.Displayable):
        def __init__(self, bait, **kwargs):
            ...
            self._bg = renpy.displayable("backgrounds/location_pier_minigame06a.jpg")
            ...
            self._aqua = renpy.displayable("buttons/fishing_fish05.png")
        ...
        def render(self, width, height, st, at):
            ...
            if not self._fish_caught:
                ...
                if not self._aqua_appeared:
                    ...
                    if not self._bait_dropped:
                        ...
                        if self._hook_ypos < self._height:
                            self._hook_ypos += (self._height - 5) / (self._bait_fall_speed * self._frames_per_second)
                        else:
                ...
                for name in fishes:
                    if name == "Tiger Fish":
                        if M_terry.get_state() == S_terry_retire and not player.has_item("tigger"):
...
screen bait_menu:
    ...
    if player.has_item("worm"):
        imagebutton:
            focus_mask True
            idle "buttons/fishing_button01a.png"
            hover HoverImage("buttons/fishing_button01a.png")
            action SetVariable("bait", "worms"), Return()
            xpos 250
            ypos 250

    if player.has_item("lure01"):
        imagebutton:
            focus_mask True
            idle "buttons/fishing_button02a.png"
            hover HoverImage("buttons/fishing_button02a.png")
            action SetVariable("bait", "standard lure"), Return()
            xpos 150
            ypos 250

    if player.has_item("special_lure"):
        imagebutton:
            focus_mask True
            idle "buttons/fishing_button04a.png"
            hover HoverImage("buttons/fishing_button04a.png")
