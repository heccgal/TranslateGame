init python:
    class PokerMinigame(renpy.Displayable):
        def __init__(self, nb_players=2, **properties):
            super(PokerMinigame, self).__init__(**properties)
            self._bg = renpy.displayable("backgrounds/location_erik_basement_poker.jpg")
        ...
        def deal_hands(self):
            ...
            for p in xrange(self.nb_players):
                self.other_players_hands["bot{}".format(p)] = self.deck.draw(5)
        
        def change_cards(self, hand, indexes):
            to_remove = [hand[i] for i in indexes]
            for r in to_remove:
                hand.remove(r)
                self.deck.cards.append(r)
            random.shuffle(self.deck.cards)
            hand.extend(self.deck.draw(len(to_remove)))
            return hand
        
        def render(self, width, height, st, at):
            args = [width, height, st, at]
            render = renpy.render(self._bg, *args)
